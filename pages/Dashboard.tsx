...
// ... StatCard and QuickAction components remain same ...
...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
      const loadData = async () => {
          ...
          const products = await db.getProductsWithBatches();
          const customers = await db.getCustomers();
          const cash = await db.getCashBalance();

          const totalSales = invoices.reduce((sum, inv) => sum + inv.net_total, 0);
          const lowStockCount = products.filter(p => p.batches.reduce((sum, b) => sum + b.quantity, 0) < 10).length;
          ...
          setStats({ totalSales, customerCount: customers.length, lowStockCount, cash, salesData });
      };
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard title={t('dash.total_sales')} value={`${currency}${stats.totalSales.toLocaleString()}`} icon={TrendingUp} color="bg-emerald-500" subtext="Lifetime Revenue" />
        <StatCard title={t('dash.cash_balance')} value={`${currency}${stats.cash.toLocaleString()}`} icon={Coins} color="bg-blue-600" subtext="Current Treasury" />
        <StatCard title={t('dash.customers')} value={stats.customerCount.toString()} icon={Users} color="bg-indigo-500" subtext="Active Partners" />
        <StatCard title={t('dash.low_stock')} value={stats.lowStockCount.toString()} icon={AlertTriangle} color="bg-amber-500" subtext="Items below 10 qty" />
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-5">
            ...
            />
            <QuickAction 
                label={t('cust.statement') || 'Customer Statement'} 
                icon={FileText} 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-gradient-to-br from-slate-900 to-slate-800 p-8 rounded-2xl shadow-xl flex flex-col justify-between text-white relative overflow-hidden">
             ...
             <div className="relative z-10">
                 ...
                 <h3 className="text-2xl font-bold mb-3">Insights & Reports</h3>
                 <p className="text-slate-300 mb-8 leading-relaxed text-sm">
                     Access detailed financial statements, profit & loss analysis, and inventory valuation reports to make better decisions.
                 </p>
             </div>
